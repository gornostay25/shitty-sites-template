...
type VenueMapProps = {
  lat: number;
  lng: number;
  /** Venue name — used as the marker title and popup heading. */
  title: string;
  /** Address line shown inside the marker popup. */
  address: string;
};
...
export default function VenueMap({ lat, lng, title, address }: VenueMapProps) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      // Esri World Dark Gray — keyless dark basemap (base + street labels).
      const tileOptions = {
        maxNativeZoom: 16,
        maxZoom: 19,
      } as const;
  ...
  return (
    <div role="region" aria-label={title} className="relative h-64 w-full sm:h-72">
