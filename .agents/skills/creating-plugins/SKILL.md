...
| **Best for**        | First-party code, reviewed bun packages   | Third-party extensions, marketplace plugins            |

### Trusted Mode

Trusted plugins are bun packages or local files added in `astro.config.mjs`. They run in-process with your Astro site.
...
- **No Portable Text block types.** PT blocks require Astro components for site-side rendering (`componentsEntry`), which are loaded at build time from bun. Sandboxed plugins are installed at runtime and can't ship components. PT blocks are a native-plugin-only feature.
