...
```bash
# Single header
bunx emdash login --url https://my-site.pages.dev \
  --header "CF-Access-Client-Id: xxx.access" \
  --header "CF-Access-Client-Secret: yyy"

# Short form
bunx emdash login -H "CF-Access-Client-Id: xxx" -H "CF-Access-Client-Secret: yyy"
...
```bash
# Basic auth
bunx emdash login --url https://example.com -H "Authorization: Basic dXNlcjpwYXNz"

# Custom auth header
bunx emdash login --url https://example.com -H "X-API-Key: secret123"
```
...
mkdir -p .emdash
bunx emdash export-seed > .emdash/seed.json
bunx emdash export-seed --with-content > .emdash/seed.json
```
...
bunx emdash types --url https://my-site.pages.dev

# Custom output path
bunx emdash types --output src/types/cms.ts
```
...
```bash
# List content
bunx emdash content list posts
bunx emdash content list posts --status published --limit 10

# Get a single item (Portable Text fields converted to markdown)
# Returns draft data if a pending draft exists
bunx emdash content get posts 01ABC123
bunx emdash content get posts 01ABC123 --raw        # skip PT->markdown conversion
bunx emdash content get posts 01ABC123 --published   # ignore pending drafts

# Create content (auto-publishes by default)
bunx emdash content create posts --data '{"title": "Hello", "body": "# World"}'
bunx emdash content create posts --file post.json --slug hello-world
bunx emdash content create posts --draft --data '...'  # keep as draft
cat post.json | bunx emdash content create posts --stdin

# Update (requires --rev from a prior get, auto-publishes by default)
bunx emdash content update posts 01ABC123 --rev MToyMDI2... --data '{"title": "Updated"}'
bunx emdash content update posts 01ABC123 --rev MToyMDI2... --draft --data '...'  # keep as draft

# Delete (soft delete)
bunx emdash content delete posts 01ABC123

# Lifecycle
bunx emdash content publish posts 01ABC123
bunx emdash content unpublish posts 01ABC123
bunx emdash content schedule posts 01ABC123 --at 2026-03-01T09:00:00Z
bunx emdash content restore posts 01ABC123
```
...
```bash
# List collections
bunx emdash schema list

# Get collection with fields
bunx emdash schema get posts

# Create collection
bunx emdash schema create articles --label Articles --description "Blog articles"

# Delete collection
bunx emdash schema delete articles --force

# Add field
bunx emdash schema add-field posts body --type portableText --label "Body Content"
bunx emdash schema add-field posts featured --type boolean --required

# Remove field
bunx emdash schema remove-field posts featured
```
...
```bash
# List media
bunx emdash media list
bunx emdash media list --mime image/png

# Upload
bunx emdash media upload ./photo.jpg --alt "A sunset" --caption "Bristol, 2026"

# Get / delete
bunx emdash media get 01MEDIA123
bunx emdash media delete 01MEDIA123
```

### Search

```bash
bunx emdash search "hello world"
bunx emdash search "hello" --collection posts --limit 5
```

### Taxonomies

```bash
bunx emdash taxonomy list
bunx emdash taxonomy terms categories
bunx emdash taxonomy add-term categories --name "Tech" --slug tech
bunx emdash taxonomy add-term categories --name "Frontend" --parent 01PARENT123
```

### Menus

```bash
bunx emdash menu list
bunx emdash menu get primary
```
...
```bash
# Pipe to jq
bunx emdash content list posts --json | jq '.items[].slug'

# Use in scripts
ID=$(bunx emdash content create posts --data '{"title":"Hello"}' --json | jq -r '.id')
```
